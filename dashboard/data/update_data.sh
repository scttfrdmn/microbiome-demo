...
update_summary_data() {
  ...
  local per_sample=$(printf "%.3f" $(echo "scale=3; $current_cost / $COMPLETED_SAMPLES" | bc -l 2>/dev/null || echo "0.025"))
...
update_resource_data() {
  ...
  if [ -n "$cpu_data" ]; then
    ...
    local new_cpu=$(echo "$last_cpu + $RANDOM % 11 - 5" | bc -l)
    local new_memory=$(echo "$last_memory + $RANDOM % 9 - 4" | bc -l)
    local new_gpu=$(echo "$last_gpu + $RANDOM % 13 - 6" | bc -l)
    
    # Ensure values are in range
    new_cpu=$(echo "$new_cpu < 40 ? 40 : ($new_cpu > 95 ? 95 : $new_cpu)" | bc -l)
    new_memory=$(echo "$new_memory < 40 ? 40 : ($new_memory > 95 ? 95 : $new_memory)" | bc -l)
    new_gpu=$(echo "$new_gpu < 0 ? 0 : ($new_gpu > 95 ? 95 : $new_gpu)" | bc -l)
    ...
    if [ $TIME_ELAPSED -gt 300 ] && [ $TIME_ELAPSED -lt 600 ]; then
      new_gpu=$(echo "$new_gpu + 30" | bc -l)
      new_gpu=$(echo "$new_gpu > 95 ? 95 : $new_gpu" | bc -l)
    fi
    
    # Create new utilization data
    local new_time=$(($(jq -r '.utilization[-1].time' $RESOURCES_FILE) + 1))
    local temp_file=$(mktemp)
    
    jq --argjson new_time "$new_time" --argjson new_cpu "$new_cpu" --argjson new_memory "$new_memory" --argjson new_gpu "$new_gpu" \
      '.utilization = (.utilization[1:] + [{"time": $new_time, "cpu": $new_cpu, "memory": $new_memory, "gpu": $new_gpu}])' \
      $RESOURCES_FILE > "$temp_file"
    
    # Update timestamp
    jq --arg timestamp "$timestamp" '.timestamp = $timestamp' "$temp_file" > $RESOURCES_FILE
